...
export function buildCharacterGeneratorTopSection(input: CharacterGeneratorTopInput): string {
  return `<section class="card"><h1>Character Generator (Advanced View)</h1><div class="notice">For regular usage, use <a href="/ui/studio">Studio</a>. This page is for advanced controls.</div>${
  ...
  )} / negative=${esc(input.negativeTermsSummary)}</div><h2>3) Candidate / Seed / HITL Options</h2><div class="grid two"><label>Reference Asset (reference mode)<select name="referenceAssetId"><option value="">(none)</option>${
