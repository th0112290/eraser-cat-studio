...
(() => {
  ...
  const renderCreationNav = () => {
      ...
      selectionObject.startsWith("asset:") && selectedAssetId
        ...
        : selectionObject.startsWith("pack:") && packId
          ...
          : selectionObject.startsWith("episode:") && currentEpisodeId()
            ? "Episode " + currentEpisodeId()
            : currentStudioObject() || "?„ìž¬ creation objectê°€ ?†ìŠµ?ˆë‹¤.";
    ...
    renderNavLinks(navPins, [readPin("asset"), readPin("pack"), readPin("run")].filter(Boolean), "Pinned reopen???„ì§ ?†ìŠµ?ˆë‹¤.");
    renderNavLinks(
      navRecents,
      readList("assets").slice(0, 2).concat(readList("packs").slice(0, 2)).concat(readList("runs").slice(0, 2)),
      "ìµœê·¼ creation reopen ë§í¬ê°€ ?„ì§ ?†ìŠµ?ˆë‹¤."
    );
  };
  ...
  const setCounter = (id, count) => {
    const el = q(id);
    if (el instanceof HTMLElement) el.textContent = String(count) + "ê°?ë¡œë“œ??;
  };
  ...
  const updateSelectionSummary = () => {
    setSignal("studio-signal-pack", selectedPack instanceof HTMLInputElement ? selectedPack.value : "", "? íƒ?????†ìŒ");
    setSignal("studio-signal-episode", episodeInput instanceof HTMLInputElement ? episodeInput.value : "", "? íƒ???í”¼?Œë“œ ?†ìŒ");
    setSignal("studio-signal-topic", topicInput instanceof HTMLInputElement ? topicInput.value : "", "ì£¼ì œê°€ ?„ì§ ?¤ì •?˜ì? ?Šì•˜?µë‹ˆ??");
  };
  ...
  const loadAssets = async () => {
    ...
    assetsBody.innerHTML = renderStateRow(4, "loading", "?ì…‹ ë¶ˆëŸ¬?¤ëŠ” ì¤?, "ìµœì‹  ?ì…‹ ?…ë ¥ ?ˆì½”?œë? ê°€?¸ì˜¤??ì¤‘ìž…?ˆë‹¤.");
    try {
      const res = await fetch("/api/assets?limit=30");
      if (!res.ok) throw new Error("?ì…‹ ëª©ë¡ ì¡°íšŒ ?¤íŒ¨: " + res.status);
      const json = await res.json();
      const list = Array.isArray(json?.data) ? json.data : [];
      setCounter("studio-assets-count", list.length);
      if (!list.length) {
        assetsBody.innerHTML = renderStateRow(4, "empty", "?ì…‹???„ì§ ?†ìŠµ?ˆë‹¤", "?ˆí¼?°ìŠ¤, ë³€??ë·? ë°°ê²½, ì°¨íŠ¸ ?ŒìŠ¤ë¥??…ë¡œ?œí•´ ?ì…‹ ?Œì´?„ë¼?¸ì„ ?œìž‘?˜ì„¸??");
        return;
      }
      ...
      assetsBody.innerHTML = renderStateRow(4, "error", "?ì…‹ ?¼ë“œë¥??¬ìš©?????†ìŒ", String(e));
    }
  };

  const loadPacks = async () => {
    if (!(packsBody instanceof HTMLElement)) return;
    packsBody.innerHTML = renderStateRow(4, "loading", "ìºë¦­????ë¶ˆëŸ¬?¤ëŠ” ì¤?, "ìµœì‹  ?ì„± ì¶œë ¥??ê°€?¸ì˜¤??ì¤‘ìž…?ˆë‹¤.");
    try {
      const res = await fetch("/api/character-packs?limit=30");
      if (!res.ok) throw new Error("ìºë¦­????ì¡°íšŒ ?¤íŒ¨: " + res.status);
      const json = await res.json();
      const list = Array.isArray(json?.data) ? json.data : [];
      setCounter("studio-packs-count", list.length);
      if (!list.length) {
        packsBody.innerHTML = renderStateRow(4, "empty", "ìºë¦­???©ì´ ?„ì§ ?†ìŠµ?ˆë‹¤", "?¤ìŒ ?©ì„ ?¤í…Œ?´ì§•??ì¤€ë¹„ê? ?˜ë©´ ìºë¦­???ì„±ê¸°ë? ?¬ì„¸??");
        return;
      }
      ...
      packsBody.innerHTML = renderStateRow(4, "error", "ìºë¦­?????¼ë“œë¥??¬ìš©?????†ìŒ", String(e));
    }
  };

  const loadEpisodes = async () => {
    if (!(episodesBody instanceof HTMLElement)) return;
    episodesBody.innerHTML = renderStateRow(4, "loading", "?í”¼?Œë“œ ë¶ˆëŸ¬?¤ëŠ” ì¤?, "ìµœì‹  ?í”¼?Œë“œ ???íƒœë¥??™ê¸°?”í•˜??ì¤‘ìž…?ˆë‹¤.");
    try {
      const res = await fetch("/api/episodes?limit=30");
      if (!res.ok) throw new Error("?í”¼?Œë“œ ì¡°íšŒ ?¤íŒ¨: " + res.status);
      const json = await res.json();
      const list = Array.isArray(json?.data) ? json.data : [];
      setCounter("studio-episodes-count", list.length);
      if (!list.length) {
        episodesBody.innerHTML = renderStateRow(4, "empty", "?í”¼?Œë“œê°€ ?„ì§ ?†ìŠµ?ˆë‹¤", "?”ìŠ¤?¨ì¹˜ ì¹´ë“œ?ì„œ ?í”¼?Œë“œë¥??ì„±???€?„ë¼???ë¦„???œìž‘?˜ì„¸??");
        return;
      }
      ...
      episodesBody.querySelectorAll("tr[data-episode-id]").forEach((row) => {
        ...
        row.addEventListener("click", () => {
          ...
          setStatus("?í”¼?Œë“œ ? íƒ: " + (episodeId || "?????†ëŠ” ?í”¼?Œë“œ"));
        });
      });
      ...
      episodesBody.innerHTML = renderStateRow(4, "error", "?í”¼?Œë“œ ?¼ë“œë¥??¬ìš©?????†ìŒ", String(e));
    }
  };

  const loadJobs = async () => {
    if (!(jobsBody instanceof HTMLElement)) return;
    jobsBody.innerHTML = renderStateRow(5, "loading", "?‘ì—… ë¶ˆëŸ¬?¤ëŠ” ì¤?, "ìµœì‹  ???”ë ˆë©”íŠ¸ë¦¬ë? ê¸°ë‹¤ë¦¬ëŠ” ì¤‘ìž…?ˆë‹¤.");
    try {
      const res = await fetch("/api/jobs?limit=30");
      if (!res.ok) throw new Error("?‘ì—… ì¡°íšŒ ?¤íŒ¨: " + res.status);
      const json = await res.json();
      const list = Array.isArray(json?.data) ? json.data : [];
      setCounter("studio-jobs-count", list.length);
      if (!list.length) {
        jobsBody.innerHTML = renderStateRow(5, "empty", "?‘ì—…???„ì§ ?†ìŠµ?ˆë‹¤", "?„ë¦¬ë·? ?Œë”, ?¼ë¸”ë¦¬ì‹œ ?¨ê³„ë¥??œìž‘???‘ì—… ?ˆì¼??ì±„ìš°?¸ìš”.");
        return;
      }
      ...
      jobsBody.innerHTML = renderStateRow(5, "error", "?‘ì—… ?¼ë“œë¥??¬ìš©?????†ìŒ", String(e));
    }
  };

  const refreshAll = async () => {
    setStatus("?ì…‹, ìºë¦­???? ?í”¼?Œë“œ, ?‘ì—…???™ê¸°?”í•˜??ì¤?..");
    await Promise.allSettled([loadAssets(), loadPacks(), loadEpisodes(), loadJobs()]);
    setStatus("?¼ë“œ ?™ê¸°???„ë£Œ. ?œë™??ê²€? í•˜ê³??¤ìŒ ?Œí¬ë²¤ì¹˜ë¥?? íƒ?˜ì„¸??");
  };
  ...
  q("studio-create-episode")?.addEventListener("click", async () => {
    try {
      const topic = topicInput instanceof HTMLInputElement && topicInput.value.trim() ? topicInput.value.trim() : "?¤íŠœ?”ì˜¤ ?”ìŠ¤?¨ì¹˜ ?í”¼?Œë“œ";
      ...
      if (!res.ok) throw new Error(await readError(res, "?í”¼?Œë“œ ?ì„± ?¤íŒ¨"));
      const json = await res.json();
      ...
      updateSelectionSummary();
      setStatus("?í”¼?Œë“œ ?ì„±?? " + (episodeId || "(id ?†ìŒ)"));
      if (episodeId) void loadEpisodeInspector(episodeId);
      void loadEpisodes();
    } catch (error) {
      setStatus("?í”¼?Œë“œ ?ì„± ?¤íŒ¨: " + String(error));
    }
  });

  q("studio-oneclick")?.addEventListener("click", async () => {
    try {
      const topic = topicInput instanceof HTMLInputElement && topicInput.value.trim() ? topicInput.value.trim() : "?¤íŠœ?”ì˜¤ ?„ë¦¬ë·??í”¼?Œë“œ";
      ...
      if (!createRes.ok) throw new Error(await readError(createRes, "?í”¼?Œë“œ ?ì„± ?¤íŒ¨"));
      ...
      setStatus("?í´ë¦??œìž‘ ?¤íŒ¨: " + String(error));
    }
  });
  ...
  q("studio-open-editor")?.addEventListener("click", () => {
    ...
    if (!episodeId) return setStatus("ë¨¼ì? episodeIdë¥??…ë ¥?˜ì„¸??");
  ...
  q("studio-enqueue-preview")?.addEventListener("click", async () => {
    try {
      const episodeId = episodeInput instanceof HTMLInputElement ? episodeInput.value.trim() : "";
      if (!episodeId) throw new Error("ë¨¼ì? episodeIdë¥??…ë ¥?˜ì„¸??");
      const res = await fetch("/api/episodes/" + encodeURIComponent(episodeId) + "/enqueue", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ jobType: "RENDER_PREVIEW" })
      });
      if (!res.ok) throw new Error(await readError(res, "?„ë¦¬ë·????±ë¡ ?¤íŒ¨"));
      const json = await res.json();
      const jobId = String(json?.data?.job?.id || "");
      if (jobId) window.location.href = "/ui/jobs/" + encodeURIComponent(jobId);
      else setStatus("?„ë¦¬ë·??Œë”ê°€ ?ì— ?±ë¡?˜ì—ˆ?µë‹ˆ??");
    } catch (error) {
      setStatus("?„ë¦¬ë·????±ë¡ ?¤íŒ¨: " + String(error));
    }
  });
