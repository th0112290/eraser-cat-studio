...
.asset-link-chip{display:inline-flex;align-items:center;padding:7px 10px;border-radius:999px;border:1px solid #d4deec;background:#fff;color:#142033;font-size:12px;font-weight:700;text-decoration:none;appearance:none;cursor:pointer}
...
export function buildAssetsPageBody(input: AssetsPageBodyInput): string {
    ...
    ? input.previews
        .map((entry) => {
          const previewBody = entry.localExists
            ? `<div class="preview-frame"><img src="${esc(entry.url)}" alt="${esc(entry.label)} ?꾨━酉? loading="lazy" width="960" height="960"/></div>`
            : `<div class="asset-empty asset-empty-inline"><strong>${t.localPreviewMissing}</strong><span>泥섎━??濡쒖뺄 異쒕젰???꾩쭅 ?앹꽦?섏? ?딆븯?ㅻ㈃ ?먮낯 ?먯뀑 URL???댁뼱 ?뺤씤?섏꽭??</span></div>`;
          ...
          }">${entry.localExists ? "以鍮꾨맖" : "?놁쓬"}</span></div>${previewBody}<p><a href="${esc(entry.url)}">${t.openPreview}: ${esc(
            entry.label
          )}</a></p></article>`;
        })
        .join("")
  ...
  const qcHeadline = input.selectedAsset
    ? /pass|ready|ok/i.test(input.selectedAsset.qcLevel)
      ? "QC媛 ?ㅼ슫?ㅽ듃由??묒뾽?쇰줈 ?섍린湲곗뿉 異⑸텇???덉젙?곸엯?덈떎."
      : /n\/a/i.test(input.selectedAsset.qcLevel)
        ? "QC媛 ?꾩쭅 紐낇솗???먯젙???대━吏 紐삵뻽?듬땲??"
        : "?멸퀎 ?꾩뿉 ?섎룄?곸씤 ?먭????꾩슂?⑸땲??"
    : "寃???먯뿉????ぉ???좏깮?섎㈃ ?먭? ?⑤꼸怨??ㅼ쓬 ?≪뀡???대┰?덈떎.";

  const qcDetail = input.selectedAsset
    ? `?곹깭 ${input.selectedAsset.status} / QC ${input.selectedAsset.qcLevel}. ?ㅼ쓬 ?④퀎濡??섍린湲????꾨━酉곗? JSON???④퍡 ?뺤씤?섏꽭??`
    : "寃???먯뿉???먯뀑???좏깮?????ㅽ뒠?붿삤濡??섎룎由댁?, 罹먮┃???앹꽦湲곗뿉 ?ъ엯?좎?, 怨꾩냽 ?먭????섏? 寃곗젙?섏꽭??";

  const inspectorRail = `<section class="card asset-route-card"><div class="asset-section-head"><div><p class="asset-kicker">Sticky Inspector</p><h2>怨좎젙 ?먭? ?덉씪</h2><p class="asset-copy">媛?대뜲 寃???먯뿉???좏깮????ぉ?????ㅻⅨ履??덉씪??怨좎젙?⑸땲?? ?먭?, ?ㅼ쓬 ?≪뀡, 留곹겕??寃쎈줈瑜????먮쫫?먯꽌 ?쎌쑝?몄슂.</p></div></div><div class="asset-link-grid"><a class="asset-link-chip" href="/ui/studio">鍮좊Ⅸ ?먮쫫? Studio</a><a class="asset-link-chip" href="/ui/characters">源딆? ??由щ럭??Characters</a></div></section>`;
  const creationNav = `<section class="card asset-route-card"><div class="asset-section-head"><div><p class="asset-kicker">Creation Handoff</p><h2>??留곹겕 / Pins / Recent</h2><p class="asset-copy">?꾩옱 asset??湲곗??쇰줈 returnTo, current object, focus瑜??좎??섍퀬 creation surfaces ?ъ씠瑜??ㅼ떆 ?щ뒗 ?덉씪?낅땲??</p></div></div><div class="asset-route-grid"><article class="asset-route-item"><strong>?꾩옱 ?ㅻ툕?앺듃</strong><p id="asset-nav-current">${input.selectedAsset ? `Asset ${esc(input.selectedAsset.id)}` : "?좏깮???먯뀑???놁뒿?덈떎."}</p><div class="asset-link-grid" id="asset-nav-actions"></div></article><article class="asset-route-item"><strong>Pinned Asset</strong><p>怨좎젙????reference asset????踰덉뿉 ?ㅼ떆 ?쎈땲??</p><div class="asset-link-grid" id="asset-pin-list"></div></article><article class="asset-route-item"><strong>Recent Assets</strong><p>理쒓렐 creation handoff?먯꽌 ?댁뿀??asset??鍮좊Ⅴ寃??ㅼ떆 ?쎈땲??</p><div class="asset-link-grid" id="asset-recent-list"></div></article></div></section>`;

  const nextActions = input.selectedAsset
    ? `<section class="card asset-next-card"><div class="asset-section-head"><div><p class="asset-kicker">?ㅼ쓬 ?≪뀡</p><h2>${t.nextActions}</h2><p class="asset-copy">${qcHeadline}</p></div></div><div class="asset-next-grid"><article class="asset-next-item"><strong>1. 異쒕젰 ?뺤씤</strong><span>${qcDetail}</span><div class="asset-link-grid"><a class="asset-link-chip" href="/api/assets/${encodeURIComponent(
        input.selectedAsset.id
      )}">${t.openJson}</a>${
        primaryPreviewHref ? `<a class="asset-link-chip" href="${esc(primaryPreviewHref)}">${t.openPreview}</a>` : ""
      }</div></article><article class="asset-next-item"><strong>2. ?먯뀑???ㅼ쓬 ?④퀎濡??쇱슦??/strong><span>${
        assetLooksReady
          ? "???먯뀑?????앹꽦 ?곗쓽 湲곗??먯씠 ??以鍮꾧? ?섏뿀?듬땲?? 鍮좊Ⅸ ?꾪솚? Studio, ?④퀎???앹꽦? Character Generator?먯꽌 ?댁뼱媛?몄슂."
          : assetQcPending
            ? "QC ?먯젙???꾩쭅 ?대젮 ?덉뒿?덈떎. 鍮좊Ⅸ ?붿뒪?⑥튂 ?꾩뿉 Studio?먯꽌 留λ씫??臾띔퀬, ?꾩슂?섎㈃ ???붾㈃??癒몃Ъ硫??먭???留덈Т由ы븯?몄슂."
            : "?ㅼ슫?ㅽ듃由쇱뿉 ?섍린湲???寃?좉? ???꾩슂?⑸땲?? ?앹꽦湲곕줈 ?섍린湲곕낫???ㅽ뒠?붿삤???꾩옱 ?먭? ?덉씪?먯꽌 癒쇱? 湲곗????뺣━?섏꽭??"
      }</span><div class="asset-link-grid"><a class="asset-link-chip" href="/ui/studio" data-asset-nav-target="studio">?ㅽ뒠?붿삤 ?닿린</a><a class="asset-link-chip" href="/ui/character-generator" data-asset-nav-target="generator">罹먮┃???앹꽦湲??닿린</a></div></article><article class="asset-next-item"><strong>3. 蹂듦뎄 留λ씫 蹂댁〈</strong><span>?뱀씤, 鍮꾧탳, 濡ㅻ갚 寃곗젙???룸컺移⑦븷 ???덈룄濡?JSON怨??꾨━酉?異쒕젰???④퍡 ?좎??섏꽭??</span><div class="asset-link-grid"><a class="asset-link-chip" href="/ui/jobs">?묒뾽 ?닿린</a><a class="asset-link-chip" href="/ui/assets?assetId=${encodeURIComponent(
        input.selectedAsset.id
      )}" data-asset-nav-target="asset">???먭? ?붾㈃ ?좎?</a></div></article></div></section>`
    : `<section class="card asset-next-card"><div class="asset-section-head"><div><p class="asset-kicker">?ㅼ쓬 ?≪뀡</p><h2>${t.nextActions}</h2><p class="asset-copy">${qcHeadline}</p></div></div><div class="asset-empty"><strong>${t.noSelectedAsset}</strong><span>${qcDetail}</span></div><div class="asset-link-grid" style="margin-top:12px"><a class="asset-link-chip" href="/ui/studio" data-asset-nav-target="studio">?ㅽ뒠?붿삤 ?닿린</a><a class="asset-link-chip" href="/ui/character-generator" data-asset-nav-target="generator">罹먮┃???앹꽦湲??닿린</a></div></section>`;

  const linkedRoutes = input.selectedAsset
    ? `<section class="card asset-route-card"><div class="asset-section-head"><div><p class="asset-kicker">Linked Routes</p><h2>?곌껐???뚰겕踰ㅼ튂</h2><p class="asset-copy">???먯뀑???대뼡 ?섏궗寃곗젙硫댁쑝濡??댁뼱吏?붿? 誘몃━ 怨좎젙???〓땲?? 鍮좊Ⅸ ?먮쫫? Studio, ?앹꽦 ?덈툕??Character Generator, 源딆? ?섎룞 寃?좊뒗 Characters媛 ?대떦?⑸땲??</p></div></div><div class="asset-route-grid"><article class="asset-route-item"><div class="preview-head"><strong>Studio / Dispatch Hub</strong><span class="asset-mini-badge ${assetLooksReady ? "asset-mini-badge-ready" : "asset-mini-badge-muted"}">${assetLooksReady ? "鍮좊Ⅸ ?꾪솚" : "留λ씫 ?뺣━"}</span></div><p>?먯뀑, ?? ?먰뵾?뚮뱶, ?묒뾽??臾띠뼱 鍮좊Ⅴ寃??꾩쭊?쒗궎???쒕㈃?낅땲?? ?뱀씤?대굹 源딆? 由щ럭??嫄곌린???④린吏 ?딆뒿?덈떎.</p><div class="asset-link-grid"><a class="asset-link-chip" href="/ui/studio" data-asset-nav-target="studio">Studio ?닿린</a><a class="asset-link-chip" href="/ui/jobs">Jobs ?닿린</a></div></article><article class="asset-route-item"><div class="preview-head"><strong>Character Generator / ?앹꽦 ?덈툕</strong><span class="asset-mini-badge ${assetLooksReady ? "asset-mini-badge-ready" : "asset-mini-badge-muted"}">${assetLooksReady ? "reference ready" : "?먭? ??沅뚯옣"}</span></div><p>???곗쓣 ?쒖옉?섍퀬, ?꾨낫瑜?鍮꾧탳?섍퀬, regenerate/recreate/rollback ?⑦꽩???댁쁺?섎뒗 ?④퀎???쒕㈃?낅땲??</p><div class="asset-link-grid"><a class="asset-link-chip" href="/ui/character-generator" data-asset-nav-target="generator">Character Generator ?닿린</a><a class="asset-link-chip" href="/ui/studio" data-asset-nav-target="studio">Studio濡??섎룎?꾧?湲?/a></div></article><article class="asset-route-item"><div class="preview-head"><strong>Characters / Pack Review</strong><span class="asset-mini-badge asset-mini-badge-muted">源딆? 寃??/span></div><p>?대? ?앹꽦???⑹쓽 preview, QC, lineage, jobs瑜??쒓납?먯꽌 ?쎌뼱???????ъ슜?섎뒗 ?섎룞 由щ럭 ?쒕㈃?낅땲??</p><div class="asset-link-grid"><a class="asset-link-chip" href="/ui/characters" data-asset-nav-target="characters">Characters ?닿린</a><a class="asset-link-chip" href="/ui/character-generator" data-asset-nav-target="generator">?뱀씤/濡ㅻ갚?쇰줈 ?대룞</a></div></article></div></section>`
    : `<section class="card asset-route-card"><div class="asset-section-head"><div><p class="asset-kicker">Linked Routes</p><h2>?곌껐???뚰겕踰ㅼ튂</h2><p class="asset-copy">?먯뀑???좏깮?섎㈃ ?ш린??Studio, Character Generator, Characters濡??댁뼱吏??寃쎈줈媛 ??援ъ껜?곸쑝濡??대┰?덈떎.</p></div></div><div class="asset-link-grid"><a class="asset-link-chip" href="/ui/studio" data-asset-nav-target="studio">Studio ?닿린</a><a class="asset-link-chip" href="/ui/character-generator" data-asset-nav-target="generator">Character Generator ?닿린</a><a class="asset-link-chip" href="/ui/characters" data-asset-nav-target="characters">Characters ?닿린</a></div></section>`;

  const selectedDetails = input.selectedAsset
    ? `<section class="card asset-detail-panel" id="asset-selected-detail"><div class="asset-section-head"><div><p class="asset-kicker">怨좎젙 ?먭? ?덉씪</p><h2>${t.selectedAsset}</h2><p class="asset-copy">?좏깮???덉퐫?쒕? ?먭??섍퀬 QC ?곹깭, 濡쒖뺄 異쒕젰, ?멸퀎 以鍮꾨룄瑜????덉씪?먯꽌 ?뺤씤?????ㅼ쓬 ?뚰겕踰ㅼ튂瑜?寃곗젙?섏꽭??</p></div><div class="asset-link-row"><a class="asset-link-chip" href="/api/assets/${encodeURIComponent(
          input.selectedAsset.id
        )}">${t.openJson}</a><button type="button" class="asset-link-chip" id="asset-copy-link">copy deep link</button><button type="button" class="asset-link-chip" id="asset-pin-current">pin current asset</button><a class="asset-link-chip" href="#" id="asset-return-link" hidden>return</a></div></div><div class="asset-summary-grid"><div class="asset-summary-card"><span>ID</span><strong>${esc(
          input.selectedAsset.id
        )}</strong></div><div class="asset-summary-card"><span>?곹깭</span><strong><span class="badge ${esc(
  ...
  return `${ASSET_PAGE_STYLE}${heroSection}<section class="asset-shell"><div class="asset-left-rail"><section class="card asset-upload-card"><div class="asset-section-head"><div><p class="asset-kicker">?낅젰</p><h2>${t.upload}</h2><p class="asset-copy">?뚯뒪 ?대?吏瑜??뚯씠?꾨씪?몄뿉 ?ｊ퀬 泥섎━ ?쒖옉怨??숈떆??諛붾줈 ?먭??쇰줈 ?대룞?⑸땲??</p></div></div><form id="asset-upload-form" enctype="multipart/form-data" class="grid"><div class="grid two"><label>?먯뀑 ???select name="assetType"><option value="character_reference">character_reference (?덊띁?곗뒪)</option><option value="character_view">character_view (酉?蹂??</option><option value="background">background (諛곌꼍)</option><option value="chart_source">chart_source (李⑦듃 ?뚯뒪)</option></select></label><label>?뚯씪<input type="file" name="file" accept="image/png,image/jpeg,image/webp" required/></label></div><button id="asset-upload-submit" type="submit" data-primary-action="1" data-primary-label="?먯뀑 ?낅줈????寃???닿린">${t.uploadAction}</button></form><p class="asset-inline-note">吏???낅젰 ?뺤떇: PNG, JPEG, WebP. ?낅줈?쒓? ?깃났?섎㈃ ???먯뀑 ?곸꽭 ?덉퐫?쒕줈 ?대룞?⑸땲??</p><pre id="asset-upload-result" class="asset-output" data-state="idle" role="status" aria-live="polite" aria-atomic="true">${t.uploadResultIdle}</pre></section><section class="card asset-guide-card"><div class="asset-section-head"><div><p class="asset-kicker">寃???덉감</p><h2>${t.reviewProtocol}</h2><p class="asset-copy">??μ냼 釉뚮씪?곗쭠蹂대떎 ?먭?怨??ㅼ쓬 寃곗젙???먯쓽 珥덉젏???좎??섏꽭??</p></div></div><ol class="asset-guide-list"><li>?좏깮???먯뀑???섎룄???덊띁?곗뒪???뚯뒪 ??븷怨??ㅼ젣濡?留욌뒗吏 ?뺤씤?⑸땲??</li><li>?앹꽦?대굹 ?ㅼ??ㅽ듃?덉씠?섏쑝濡??섍린湲??꾩뿉 QC ?깃툒怨??꾨━酉?異쒕젰???뺤씤?⑸땲??</li><li>?뱀씤, 鍮꾧탳, 濡ㅻ갚 ?먮떒???꾩슂??寃쎌슦 JSON怨??꾨━酉곕? 蹂듦뎄 留λ씫?쇰줈 ?④퍡 ?ъ슜?⑸땲??</li></ol></section></div><div class="asset-main-col"><section class="card asset-list-card"><div class="asset-section-head"><div><p class="asset-kicker">寃????/p><h2>${t.recentAssets}</h2><p class="asset-copy">吏湲??먭????꾩슂???먯뀑??怨좊Ⅴ怨? ?ㅻⅨ履?sticky inspector?먯꽌 ?ㅼ쓬 議곗튂瑜?寃곗젙?섏꽭??</p></div><span id="asset-filter-count" class="asset-counter">0 / 0媛??쒖떆 以?/span></div><div class="asset-table-tools"><input id="asset-filter" type="search" autocomplete="off" aria-label="理쒓렐 ?먯뀑 ?꾪꽣" placeholder="${t.filterPlaceholder}" /><span class="asset-filter-note">?꾩옱 濡쒕뱶???됱뿉留??곸슜?섎뒗 濡쒖뺄 ?꾪꽣?낅땲??</span></div><div class="asset-table-wrap"><table id="asset-table"><thead><tr><th>${t.columns.id}</th><th>${t.columns.type}</th><th>${t.columns.status}</th><th>${t.columns.qc}</th><th>${t.columns.mime}</th><th>${t.columns.size}</th><th>${t.columns.created}</th></tr></thead><tbody>${
    input.rows || renderTableEmptyRow(7, t.noAssets)
  }</tbody></table></div></section></div><div class="asset-right-rail">${inspectorRail}${creationNav}${selectedDetails}${nextActions}${linkedRoutes}</div></section>${clientScript}${creationNavScript}`;
}
