...
export function registerApiRoutes(input: RegisterApiRoutesInput): void {
  ...
  app.get("/ui/assets", async (request, reply) => {
    try {
      ...
      const previews = selected && canPreviewImage
        ? [
            ...
            .map((entry) => ({
              label: previewLabel(entry.label),
              key: String(entry.key),
              url: String(entry.url),
              localExists: storageKeyExistsLocally(String(entry.key))
            }))
        : [];

      const selectedQcBadge = selected ? getQcBadgeSummary(selected.status, selected.qcJson) : null;
      const body = buildAssetsPageBody({
        rows,
        selectedAsset: selected
          ? {
              id: selected.id,
              status: selected.status,
              statusClassName: uiBadgeClass(selected.status),
              mime: selected.mime ?? "-",
              originalKey: selected.originalKey ?? selected.storageKey ?? "-",
              normalized1024Key: selected.normalizedKey1024 ?? "-",
              normalized2048Key: selected.normalizedKey2048 ?? "-",
              qcLevel: selectedQcBadge?.level ?? "N/A",
              qcClassName: selectedQcBadge?.className ?? "muted",
              qcReason: selectedQcBadge?.reason ?? "-",
              qcJson: JSON.stringify(selected.qcJson ?? null, null, 2)
            }
          : null,
        previews
      });
