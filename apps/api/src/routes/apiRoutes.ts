...
export function registerApiRoutes(input: RegisterApiRoutesInput): void {
  ...
  app.get("/ui/assets", async (request, reply) => {
    ...
    const body = `<section class="card"><h1>Assets</h1><form id="asset-upload-form" enctype="multipart/form-data" class="grid"><div class="grid two"><label>assetType<select name="assetType"><option value="character_reference">character_reference (레퍼런스)</option><option value="character_view">character_view (뷰 변형)</option><option value="background">background (배경)</option><option value="chart_source">chart_source (차트 소스)</option></select></label><label>file<input type="file" name="file" accept="image/png,image/jpeg,image/webp" required/></label></div><button id="asset-upload-submit" type="submit">Upload + Enqueue ASSET_INGEST</button></form><pre id="asset-upload-result"></pre></section><section class="card"><h2>Latest Assets</h2><table><thead><tr><th>ID</th><th>assetType</th><th>Status</th><th>QC</th><th>MIME</th><th>Bytes</th><th>Created</th></tr></thead><tbody>${rows || '<tr><td colspan="7">No assets</td></tr>'}</tbody></table></section>${selectedDetails}<script>const form=document.getElementById(\"asset-upload-form\");const output=document.getElementById(\"asset-upload-result\");const submit=document.getElementById(\"asset-upload-submit\");if(form&&output&&submit){form.addEventListener(\"submit\",async(event)=>{event.preventDefault();submit.disabled=true;output.textContent=\"Uploading...\";const fd=new FormData(form);try{const res=await fetch(\"/api/assets/upload\",{method:\"POST\",body:fd});const json=await res.json();output.textContent=JSON.stringify(json,null,2);if(res.ok&&json&&json.data&&json.data.assetId){window.location.href=\"/ui/assets?assetId=\"+encodeURIComponent(json.data.assetId);} }catch(error){output.textContent=String(error);}finally{submit.disabled=false;}});}</script>`;
