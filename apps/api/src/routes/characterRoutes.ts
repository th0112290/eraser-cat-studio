...
function buildCharacterPackLineageSection(input: {
  ...
  if (!input.lineage) {
    const fallbackLinks = input.compareHref ? [{ label: "Compare vs active pack", href: input.compareHref }] : [];
    ...
    )}</strong>. This page only reads existing generated pack artifacts; it does not rerun generation.</div>${renderLineageLinks(fallbackLinks)}</section>`;
  }
