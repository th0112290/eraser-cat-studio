...
function renderWorkflowSampleArtifactsFallback(): string {
  ...
  return `<section class="card"><h2>Local Workflow Samples</h2><div class="notice">DB?∂Ïéõ? ??Í≥∑ÏÑ†???????Á≠åÏöä?????Î∞¥Ïâê??ComfyUI workflow JSON?????????∞Î????àÎºÑ. <strong>workflow_gui.json</strong>?????±Îí≤?•‚â™ÎÆÜË´≠??Í≥¥Ìê£ ComfyUI Á≠??Ä????∞Ìì† ??Î∫§ÏÇã?üÎ∞∏Ï±∏Èáâ?Ï≠??ÔßèÍªäÍµ??üÎ∞∏Ï±??Ë¢? ?∞Í∑£?????àÎºÑ.</div>${
    compareReportUrl
...
export function registerCharacterRoutes(input: RegisterCharacterRoutesInput): void {
  ...
  app.get("/ui/character-generator", async (request, reply) => {
    ...
    const selectedSection = selectedJob
      ? `<section class="card" id="cg-active-job"><div id="cg-active-job-meta" hidden data-current-run-id="${escHtml(
          selectedJob.id
        )}" data-current-pack-id="${escHtml(selectedManifest?.characterPackId ?? "")}"></div><h2>Generation Run object</h2><p>Stage 04??????????Generation Run object???ÖÎπç?? ??®Î∞¥Î¨?? ?Ë¢Å„Çã????ÎØ™Îàò? ??Íæ™¬Ä? next safe action??linked routes????•Ï•ô?????Compare?? Approve/Rollback ???±Îµ•??Í≥óÏ®Æ ???æ‚îõ?ÔßèÍæ©??</p><div class="cg-context-grid">${renderGeneratorObjectCard(
          "Generation Run",
        ...
        )}</p>${selectedPrimaryActionControl}</article><article class="cg-context-card"><h3>Linked routes</h3><p><strong>Follow the object, not the page.</strong></p><p>Studio????Ï•????ÍπÜÎí≠???ÜÏ∂∏ Á≠åÎùØ??? compare/approve/rollback?? ?Ë¢Å‚ë•???Ë¢Å‚ëπ??surface????????ÎÆ???àÎºÑ.</p><div class="cg-link-list">${selectedLinkedRoutes
          .map((link) => `<a href="${escHtml(link.href)}">${escHtml(link.label)}</a>`)
    ...
    const recommendedActionsSection =
      selectedJob && selectedManifest
        ? `<section class="card" id="recommended-actions"><h2>Next Safe Actions</h2><p>?????±Îµ•?? compare ??Íæ©Îúé???Ï∂????àÎÆâ ???±Ïùà?????±Î≤â ??Î§øÎñÑ????? Candidate set??Character Pack object È§???úÎòªÊØ????Á≠åÏöä???èÏ?? ?ÎØ™Îàò? ??Î∫§Ïëé???±¬Ä? ÁππÎ®ÆÎ¥? preview/QC/lineage ???Îº?? Characters surface?????ÅÎè•???àÎºÑ.</p>${
    ...
    const pickSection =
      selectedJob && selectedManifest
        ? (() => {
            ...
            return `<section class="card" id="pick-candidates"><h2>Candidate Set / HITL compare</h2><p>?????Ä??candidate set??????????Character Pack build?????∞ÏÑ†???∫Í≥å?¨ÈçÆ???Á≠åÎ§øÍµ???®Î™ÑÎ™µÂö•???´Î§æÎ¨??Î™ÉÎπç?? ?????Compare?????ç¬Ä???Î°?ä∫ approval lane??Í≥óÏ®Æ ??????Î™ÉÎπç??</p>${
              pickBlocked
          ...
          }>??´Î§æÎ¨???®Î™Ñ??+ Character Pack build</button></div></form>${candidateCardsForView(
            "front"
    ...
    const previewSection =
      selectedManifest && selectedManifest.characterPackId
        ? (() => {
            ...
            return `<section class="card" id="pack-preview-handoff"><h2>Character Pack object / handoff</h2><p>compare???????? Character Pack object??review?? active-baseline approval?????æ‚îõ??handoff surface???ÖÎπç?? ??ÁππÎ®ÆÎ¥? preview/QC/lineage/jobs inspection?? Characters????????ÎÆ???àÎºÑ.</p><p>characterPackId: <a href="/ui/characters?characterPackId=${encodeURIComponent(
              selectedManifest.characterPackId
              ...
              selectedManifest.characterPackId
            )}"/>${selectedPackGeneratorNavFields}<button type="submit" class="secondary">????Î±ÄÎ±?active baseline??Í≥óÏ®Æ ?Ë´?Ä??/button></form></section>`;
          })()
        : "";

    const rollbackSection =
      approvedPacks.length > 0
        ? `<section class="card" id="rollback-active-pack"><h2>Character Pack / rollback active baseline</h2><p>?Ë¢Å‚ëπ??active baseline????Í≥∏Ïùà approved Character Pack??Í≥óÏ®Æ ??Î°´Ï¶º?ÍπÜÎü•??? compare?? review????Î©∏Îïã ?? ?Ë¢Å‚ëπ??active pack?∞Í∑£??????±Ïùà????Í≥∏Ïùà ?´Íø∏?????Î∏åÏë¨Ôß?????Ï∂??????Î§æÏâ≠??</p><form method="post" action="/ui/character-generator/rollback-active" class="grid two">${selectedPackGeneratorNavFields}<label>Target Pack<select name="targetCharacterPackId">${approvedPacks
            .map(
              (pack) =>
                `<option value="${escHtml(pack.id)}">${escHtml(pack.id)} (v${escHtml(pack.version)}, ${escHtml(pack.status)})</option>`
            )
            .join("")}</select></label><div class="actions" style="grid-column:1/-1"><button type="submit" class="secondary">Active baseline ?•‚ñ≤?áÂ™õ?/button></div></form></section>`
    ...
    const compareSection =
      approvedPacks.length >= 2
        ? `<section class="card" id="compare-approved-packs"><h2>Character Pack / compare approved baselines</h2><p>?Ë´?Ä???Character Pack baseline??Í≤∏Î¥∫ preview/QC/lineage/jobs ??®¬Ä??£Ïë¨? ???àÎÆâ dedicated compare surface?????àÎπç??</p><form method="get" action="/ui/character-generator/compare" class="grid two"><input type="hidden" name="returnTo" value="${escHtml(currentPageReturnTo)}"/><input type="hidden" name="currentObject" value="${escHtml(selectedManifest?.characterPackId ? `pack:${selectedManifest.characterPackId}` : generatorSelfNav.currentObject ?? "")}"/><input type="hidden" name="focus" value="pack-compare-hero"/><label>Left Pack<select name="leftPackId">${approvedPacks
            ...
            .join("")}</select></label><div class="actions" style="grid-column:1/-1"><button type="submit" class="secondary">Approved Pack A/B compare ???©‚îõ</button></div></form></section>`
  ...
  app.get("/ui/characters", async (request, reply) => {
    ...
    const charactersNavSection = `<section class="pack-review-rail" id="pack-review-creation-nav" data-current-pack-id="${escHtml(
      selectedPack?.id ?? ""
    )}"><p class="eyebrow">Creation Handoff</p><h2>Return / current pack / reopen</h2><p>Characters??pack detail surface?ÖÎãà?? compare, lineage, generator approval lane, studio dispatch hubÎ°??§Ïãú ?§Ïñ¥Í∞????ÑÏû¨ pack deep linkÎ•?Î®ºÏ? Í≥†Ï†ï?©Îãà??</p><div class="pack-review-actions"><a class="pack-review-link" href="${escHtml(
