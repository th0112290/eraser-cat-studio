...
const MASCOT_VIEW_MODIFIERS: Record<CharacterView, string> = {
  ...
  threeQuarter:
    "strict three-quarter view around 35 to 45 degrees, head and torso clearly turned together, both eyes visible but far-side features smaller, far ear slightly occluded, near cheek and near muzzle plane dominant, near shoulder forward and far shoulder pulled back, chest and hips rotated off-axis, near torso contour wider than the far side, far paw smaller but still readable, preserve compact mascot head silhouette, tiny body, same family proportions, clear torso yaw, do not keep both shoulders square to camera, no near-front cheat, no near-profile collapse, no duplicate face parts, no human facial structure, no straight-on symmetry",
  profile:
...
export const STYLE_PROMPT_PRESETS: StylePromptPreset[] = [
  {
    ...
    positive: [
      ...
      "tiny simple body under the head",
      "short stubby limbs",
      ...
      "naive hand-drawn doodle finish",
    ...
    negative: [
      ...
      "rounded soft puppy proportions",
      "realistic fur",
      "grey fur shading",
      "anime shading",
      "gradient shading",
    ...
    ? [
        ...
        ...(premiumCatCanonPreset || premiumMascotBankPreset
          ? [
              ...
              "keep the outline thick and slightly uneven instead of polished",
              "keep the fill almost pure black and white with no rich shading",
              "keep the face deadpan and minimally expressive",
              "avoid plush softness and avoid sticker-border presentation"
            ]
          : []),
        ...(premiumCatCanonPreset
          ? [
